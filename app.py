...
#from resume import process_resumes_and_shortlist
...
def list_cosine_sim(list1, list2):
    list1_text = " ".join([s.lower().strip() for s in list1 if s.strip()])
    list2_text = " ".join([s.lower().strip() for s in list2 if s.strip()])
    ...
    try:
        ...
        # If CountVectorizer fails (e.g., due to empty vocabulary after filtering)
        return 0.0
...
def process_resumes_and_shortlist(job_description, resume_texts, top_n):
    all_results = []

    for filename, filepath in resume_texts:
        resume_text = extract_resume_text(filepath)

        # Prepare input state for graph
        state = {
        ...
        # Get scores
        analyst_score = final_state.get("analyst_score", 0)
        skills_score = final_state.get("skills_score", 0)
        education_score = final_state.get("education_score", 0)
        hr_score = final_state.get("hr_score", 0)

        total_score = (analyst_score + skills_score + education_score + hr_score) / 4
        ...
        all_results.append({
            ...
            "feedback": final_state.get("feedback", "No feedback")
        })

    sorted_resumes = sorted(all_results, key=lambda x: x["final_score"], reverse=True)
...
def shortlist():
    try:
        ...
        for file in files:
            ...
            file.save(filepath)
            resume_texts.append((filename, filepath))

        # Process resumes using your own function
        shortlisted = process_resumes_and_shortlist(job_description, resume_texts, top_n)

        # Return the response
        return jsonify({
            "message": "Resumes shortlisted successfully",
            "resumes": shortlisted
        })

    except Exception as e:
        print("Error in /shortlist:", str(e))
        return jsonify({"error": str(e)}), 500

@app.route("/download_top_n", methods=["POST"])
def download_top_n():
    try:
        resumes_data = request.json.get("resumes", [])
        zip_filename = "shortlisted_resumes.zip"
        zip_path = os.path.join(tempfile.gettempdir(), zip_filename)

        with zipfile.ZipFile(zip_path, "w") as zipf:
            for item in resumes_data:
                filename = item.get("filename")
                path = item.get("path")  # If you're saving filepath
        ...
        print("Error in /download_top_n:", str(e))
        return jsonify({"error": str(e)}), 500
